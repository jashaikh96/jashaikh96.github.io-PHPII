...
$sql = "INSERT INTO members(id, name, dob, gender, SSN, address, phone, email, password) values ('{$userid}', '{$name}', '{$dob}', '{$gender}', '{$ssn}', '{$address}', '{$phone}', '{$email}', '{$password}')";

$res = $conn->query($sql);

if ($res) {

header("location: admin.php?msg=registered successfully!");

} else {

header("location: member_registration.php?msg=unable to register!");
...
<button type="submit" name="member_regis">Register</button>
...
<h4>Already registered? <a href="member_login.php">Log In here</a></h4>
